...
def get_start_message(message):
    ...
    btn4 = types.KeyboardButton('/goroskop')

    markup.row(btn1, btn2, btn3, btn4)
    bot.send_message(message.from_user.id,
                        "Привет, {0.first_name}! Я, Даша-путешественница, помогу тебе провести незабываемое время в Москве! По команде: \n/help можно станцию метро\n/weather можно узнать о погоде в Москве\n/add ты можешь добавить места, которых нет в списке, чтобы поделиться своими впечатлениями, а мы их добавим в список\n/goroskop ты можешь узнать свой гороскоп на день".format(
...
@bot.callback_query_handler(func=lambda call: len(call.data.split(',')) == 2)#принимает анонимную функциюи возвращает True
...
def add_review(message):
    ...
    bot.send_message(message.chat.id, 'Спасибо, отзыв добавлен! Хочешь прочитать отзывы других людей? [Да/Нет]:')
    bot.register_next_step_handler(message, read_reviews)
...
def callback_worker1(call):
    ...
    if call.data == "zodiac":
        # Формируем гороскоп
        msg1 = random.choice(first) + ' ' + random.choice(second) + ' ' + random.choice(second_add) + ' ' + random.choice(third) + ' ' + random.choice(fourth)
